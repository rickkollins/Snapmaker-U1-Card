  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .card-glow-border {
    ...
    background: linear-gradient(135deg,
      rgba(123,97,255,0.8) 0%, rgba(0,229,255,0.55) 25%,
      rgba(191,90,242,0.7) 50%, rgba(0,229,255,0.45) 75%,
      rgba(123,97,255,0.8) 100%);
  ...
  .hdr-brand {
    ...
    color: rgba(180,190,255,0.4); font-weight: 600; line-height: 1; margin-bottom: 2px;
  }
  ...
  .prism-line {
    ...
    background: linear-gradient(90deg,
      transparent 0%, #7b61ff 15%, #00e5ff 38%,
      #bf5af2 58%, #ff6b8a 74%, #7b61ff 88%, transparent 100%);
  ...
  /* ── Content row ── */
  .content-row { display: flex; align-items: stretch; background: #080a10; }
  ...
  .side-stat-label { font-size: 8px; text-transform: uppercase; letter-spacing: 0.12em; color: rgba(150,160,220,0.38); font-weight: 700; line-height: 1; }
  ...
  .hot-val   { color: #ff8c6b; } .warm-val { color: #fbbf24; } .cool-val { color: rgba(150,170,230,0.45); }
  .layer-val { color: #a78bfa; } .speed-val { color: #38bdf8; } .fan-val { color: #34d399; }
  ...
  .center-img { width: 100%; height: 100%; object-fit: contain; object-position: center; display: block; filter: brightness(0.9) contrast(1.04); }
  .img-inner-glow {
  ...
  .mmu-label { font-size: 8px; text-transform: uppercase; letter-spacing: 0.16em; color: rgba(150,165,220,0.35); font-weight: 700; flex-shrink: 0; }
  ...
  .prog-file { font-family: 'JetBrains Mono', monospace; font-size: 10px; color: rgba(160,180,255,0.45); letter-spacing: 0.05em; margin-bottom: 8px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .prog-row { display: flex; align-items: center; gap: 10px; margin-bottom: 10px; }
  .prog-pct { font-family: 'JetBrains Mono', monospace; font-size: 24px; font-weight: 700; line-height: 1; flex-shrink: 0; filter: drop-shadow(0 0 6px rgba(0,229,255,0.35)); }
  .prog-pct.cyan   { background: linear-gradient(135deg,#7b61ff,#00e5ff); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
  .prog-pct.amber  { background: linear-gradient(135deg,#f59e0b,#fbbf24); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; filter: drop-shadow(0 0 6px rgba(245,158,11,0.4)); }
  .prog-pct.red    { background: linear-gradient(135deg,#ff4f4f,#ff8c6b); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; filter: drop-shadow(0 0 6px rgba(255,79,79,0.4)); }
  ...
  .prog-time-label { font-size: 9px; text-transform: uppercase; letter-spacing: 0.09em; color: rgba(150,165,220,0.38); font-weight: 700; }
...
class SnapmakerU1Card extends HTMLElement {
  ...
  static getConfigElement() {
    return null; // no visual config editor
  }
  ...
  async _poll() {
    ...
    const fields = 'print_stats&extruder&heater_bed&fan&virtual_sdcard&motion_report&gcode_move';
    try {
  ...
  _attachListeners() {
    ...
    root.querySelector('.btn-clear')   ?.addEventListener('click', () => this._apiPost('/machine/proc_stats')); // clears error
    ...
    root.querySelector('.btn-preheat') ?.addEventListener('click', () => {
      // Set extruder to 200°C and bed to 60°C as a preheat default
      fetch(`${this._config.moonraker_url}/printer/gcode/script`, {
        method: 'POST',
        headers: this._headers({ 'Content-Type': 'application/json' }),
        body: JSON.stringify({ script: 'M104 S200\nM140 S60' }),
        signal: AbortSignal.timeout(5000),
      }).catch(() => {});
    });
  }
    ...
    return `
      <div class="side-col left-col">
        <div class="side-stat-group">
          ...
          <div class="side-stat-val ${cold ? 'cool-val' : 'hot-val'}">${extActual.toFixed(0)}°</div>
          <div class="side-stat-target">${cold ? 'Off' : '/' + extTarget.toFixed(0) + '°C'}</div>
          ${showBars ? `<div class="side-mini-bar"><div class="side-mini-fill ${cold ? 'cool-fill' : 'hot-fill'}" style="height:${extPct}%"></div></div>` : ''}
        </div>
        <div class="side-divider-h"></div>
        <div class="side-stat-group">
          <div class="side-stat-label">BED</div>
          <div class="side-stat-val ${cold ? 'cool-val' : 'warm-val'}">${bedActual.toFixed(0)}°</div>
          <div class="side-stat-target">${cold ? 'Off' : '/' + bedTarget.toFixed(0) + '°C'}</div>
          ${showBars ? `<div class="side-mini-bar"><div class="side-mini-fill ${cold ? 'cool-fill' : 'warm-fill'}" style="height:${bedPct}%"></div></div>` : ''}
        </div>
      </div>`;
  }
  ...
  _mmuStrip(activeIdx = 0, showActive = true) {
    const slots = this._config.mmu_slots;
    const html = slots.map((s, i) => {
      const isActive = showActive && i === activeIdx;
      return `
        <div class="mmu-slot ${isActive ? 'active' : 'idle'}">
          <div class="mmu-swatch" style="background:${s.color};${isActive ? `box-shadow:0 0 8px ${s.color}99` : ''}"></div>
          <div class="mmu-slot-num">${i + 1}</div>
          <div class="mmu-slot-type">${s.type}</div>
          ${isActive ? `<div class="mmu-active-dot" style="background:${s.color}"></div>` : ''}
        </div>`;
    }).join('');
    return `<div class="mmu-strip"><div class="mmu-label">MMU</div><div class="mmu-slots">${html}</div></div>`;
  }
      ...
      <div class="content-row">
        ...
        <div class="side-col right-col">
          <div class="side-stat-group">
            ...
            <div class="side-stat-val layer-val">${d.currentLayer}</div>
            <div class="side-stat-target">/${d.totalLayers}</div>
          </div>
          <div class="side-divider-h"></div>
          <div class="side-stat-group">
            <div class="side-stat-label">SPEED</div>
            <div class="side-stat-val speed-val">${d.speed}</div>
            <div class="side-stat-target">%</div>
          </div>
          <div class="side-divider-h"></div>
          <div class="side-stat-group">
            <div class="side-stat-label">FAN</div>
            <div class="side-stat-val fan-val">${d.fanSpeed}</div>
            <div class="side-stat-target">%</div>
          </div>
        </div>
      </div>
      ${this._mmuStrip(0, true)}
